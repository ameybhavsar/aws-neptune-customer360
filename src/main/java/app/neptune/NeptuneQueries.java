...
public class NeptuneQueries {
    ...
    public static final String GET_CARD_RECOMMENDATION =
            "MATCH (node:Customer)-[r:HAS_CARD]-(node2) WHERE node.value = '%s' RETURN ID(node) AS id, LABELS(node) AS labels, node.value AS value, ID(node2) AS id2, LABELS(node2) AS labels2, node2.value AS value2, TYPE(r) AS type";
